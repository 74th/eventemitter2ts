...
export interface EventAndListener<K, T> {
	(event: K | K[], arg: T): void;
}

export declare class EventEmitter2<T> {
	constructor(options?: ConstructorOptions)
	emit<K extends keyof T>(event: K | K[], arg: T[K]): boolean;
	emitAsync<K extends keyof T>(event: K | K[], arg: T[K]): Promise<T[K]>;
	...
	prependListener<K extends keyof T>(event: K | K[], listener: Listener<T[K]>): this;
	...
	many<K extends keyof T>(event: K | K[], timesToListen: number, listener: Listener<T[K]>): this;
	prependMany<K extends keyof T>(event: K | K[], timesToListen: number, listener: Listener<T[K]>): this;
	onAny<K extends keyof T>(listener: EventAndListener<K, T[K]>): this;
	prependAny<K extends keyof T>(listener: EventAndListener<K, T[K]>): this;
	offAny<K extends keyof T>(listener: Listener<T[K]>): this;
	removeListener<K extends keyof T>(event: K | K[], listener: Listener<T[K]>): this;
	off<K extends keyof T>(event: K, listener: Listener<T[K]>): this;
	removeAllListeners<K extends keyof T>(event?: K | eventNS): this;
	setMaxListeners<K extends keyof T>(n: number): void;
	eventNames(): string[];
	listeners<K extends keyof T>(event: K | K[]): Listener<K>[] // TODO: not in documentation by Willian
